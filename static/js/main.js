(function ($) {
    ...
    /* dark-mode */
    // Function to set dark mode state in localStorage
    ...
    var darkMode = getDarkModeState();

    // Function to update dark mode styles
    function updateDarkModeStyles() {
        var iconElement = $(".mode-toggle i");

        if (darkMode) {
            // Dark mode
            // iconElement.removeClass("ri-contrast-2-line").addClass("ri-contrast-2-fill");
            iconElement.css({"color": "white", "font-size": "25px"});
            $("head").append('<link rel="stylesheet" class="dark-link-mode" href="static/css/dark.css" id="add_dark_mode">');
        } else {
            // Light mode
            // iconElement.removeClass("ri-contrast-2-fill").addClass("ri-contrast-2-line");
            iconElement.css({"color": "", "font-size": "25px"});
            $("#add_dark_mode").remove();
        }
    }

    // Initialize dark mode styles on page load
